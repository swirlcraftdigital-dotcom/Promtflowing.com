...
app.post('/api/trigger-simulated-receipt', async (req, res) => {
    return res.status(410).json({ error: 'Simulated checkout is disabled in production.' });
});
...
app.post('/api/create-checkout-session', async (req, res) => {
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            success_url: urls.successUrl.includes('{CHECKOUT_SESSION_ID}') ? urls.successUrl : `${urls.successUrl}${urls.successUrl.includes('?') ? '&' : '?'}checkout_success=true&session_id={CHECKOUT_SESSION_ID}`,
